...
function [S1,t,Y,lbl]=signal_generator_2d(SNR,iteraciones,T,rTipoSig,BWc,T1_rel,Tcesc,Dfesc,vsFSK,Dffsk,nFSK,vsPSK,nPSK,pAl,cAl,cod,vsASK,vsQAM,Roff)
    ...
    %Salidas:
    Xs = zeros(length(SNR)*iteraciones,64,64,2);
    %%Xw = zeros(length(SNR)*iteraciones,64,64,1);
    Xc = zeros(length(SNR)*iteraciones,64,64,1);
                       ...
                       if (~isempty(cod)),
                        switch cod,
                            ...
                            case 10,
                                ...
                                T_k = round((256 + (T(2) - 256)*rand(1)))/fs;
                                T_k=round(T_k*fs);
...
%                     Roff=[];
         ...
         %  save('Pruebas_modulaciones/BPSK_18_8', 'I','Q')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   Y -> mod, SNR, params:
         ...
         s = I + 1j*Q;
         [S1,t,f] = tfrstft(s.',1:16:length(s), length(s)/16);
         %%S2 = tfrwv(s.',1:16:length(s), length(s)/16);
         %%S3 = tfrcw(s.',1:16:length(s), length(s)/16);
         %Xs(ik,:,:,1) = abs(S1)/max(max(abs(S1)));
         %Xs(ik,:,:,2) = angle(S1)/pi;
         %%Xw(ik,:,:,1) = S2/max(max(S2));
         %Xc(ik,:,:,1) = S3/max(max(S3));
