...
function [X,Corr,Y,lbl]=signal_generator2(SNR,iteraciones,T,rTipoSig,BWc,T1_rel,Tcesc,Dfesc,vsFSK,Dffsk,nFSK,vsPSK,nPSK,pAl,cAl,cod,vsASK,vsQAM,Roff)
...
%                   Df_k=Dffsk(randsrc(1,1,[1:length(Dffsk)]));
                    ...
                    fs=1;
                    fo_k = [fs*0.03, fs*0.3]; 
                    fo_k = fo_k(1) + (fo_k(2)-fo_k(1))*rand(1);
                   
                    [x,t,codigo,error]=m_fsk(1,fo_k,Df_k,ns_k,numSimbolos_k,pAl,cAl,codFSK,nFSK_k,T_k,1,faseContinua,[],1);
...
%            length(B)
           ...
           Corr(ik, 1:T_k, 1) = Xcorr_real;
           Corr(ik, 1:T_k, 2)= Xcorr_imag;
